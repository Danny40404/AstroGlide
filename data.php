...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $token = $_POST['datamaxfallspeed'] ?? '';

    $receivedHash = hash_hmac('sha256', $name . $punkte . $timestamp, $token);
    $expectedHash = hash_hmac('sha256', $name . $punkte . $timestamp, 15);

    if ($expectedHash !== $receivedHash) {
        echo json_encode(['status' => 'error', 'message' => 'Score konnte nicht gespeichert werden.']);
        exit;
    }

    $date = date('Y-m-d H:i:s');

    if (in_array($name, $blacklist) || !isValidName($name)) {
        echo json_encode(['status' => 'error', 'message' => 'Ungültiger Name']);
        exit;
    } else {
        $sql = $conn->prepare("INSERT INTO astroglide (`Name`, `Score`, `Datum`) VALUES (?, ?, ?)");
        $sql->bind_param("sis", $name, $punkte, $date);

        if ($sql->execute()) {
            echo json_encode([
                'status' => 'sucess', 'message' => 'Der Score wurde gespeichert.'
            ]);
        } else {
            echo json_encode([
                'status' => 'error',
                'message' => 'Score konnte nicht gespeichert werden.'
            ]);
        }
        $sql->close();
    }
    ...
    if ($result) {
        ...
        while($row = $result->fetch_assoc()) {
            $array[] = array("Name" => $row["Name"], "Score" => intval($row["Score"]));
        }

        echo json_encode($array);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Fehler beim Abrufen des Highscores.']);
    }
}
$conn->close();
?>